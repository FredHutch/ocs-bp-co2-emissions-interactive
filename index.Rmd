...
knitr::include_graphics("www/img/climate_change_health_impacts.jpg")
```
...
1. Change row names of `mtcars` to the first column with column name `car` (Use the `rownames_to_column` function from the `tibble` package. See documentation [here](https://tibble.tidyverse.org/reference/rownames.html){target="_blank"}.). Name this new dataset `new.mtcars`.
2. Only keep the `car`, `mpg`, and `cyl` columns of this `new.mtcars` dataset.
3. Only keep the cars that have `cyl` = 4.
4. Rearrange the dataset by `mpg` in descending order.
...
# step 1:
# The "var" argument takes the name of the column where we put the row names. What is the column name?
```
...
# step 4:
# Which function of the "dplyr" package reorder the data?
# What argument do we need to arrange in descending order?
```
